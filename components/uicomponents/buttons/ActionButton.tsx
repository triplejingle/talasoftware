...
export function PrimaryActionButton(properties:ButtonProps) {
  const props = {...properties, className:" bg-action-600 hover:bg-action-500 text-white rounded-md "+properties.className};
...
export function SecondaryActionButton(properties:ButtonProps) {
  const props = {...properties, className:" text-action-500 border-action-300 border-2 rounded-md "+properties.className};
  return <BaseButton {...props}>
          {props.children}
        </BaseButton>
}
